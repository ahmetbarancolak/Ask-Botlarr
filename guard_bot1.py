...
class GuardBot1(commands.Cog):
    ...
    async def connect_to_voice_channel(self):
        ...
        try:
            ...
            # Cooldown kontrolü (30 saniye bekleme)
            now = asyncio.get_event_loop().time()
            last_attempt = self.last_connection_attempt.get(guild_id, 0)
            if now - last_attempt < self.connection_cooldown:
                elapsed = int(now - last_attempt)
                logger.debug(f"⏳ Bağlantı cooldown: {elapsed}/{self.connection_cooldown}s")
                return
            ...
            if not channel:
                ...
                logger.info(f"🔍 Sunucudaki kanal ID'leri: ")
            ...
            self.voice_connect_attempts[guild.id] = 0  # Reset retry counter
            logger.info(f"✅ 🎙️  {channel.name} kanalına başarıyla bağlandı")
            
        except discord.ClientException as e:
            logger.error(f"❌ Discord bağlantı hatası: {str(e)[:100]}")
            attempts = self.voice_connect_attempts.get(guild_id, 0)
            if attempts < 2:
                self.voice_connect_attempts[guild_id] = attempts + 1
                logger.info(f"🔄 30 saniye sonra yeniden bağlanmaya çalışılacak ({attempts + 1}/2)")
        except asyncio.TimeoutError:
            logger.error("❌ Bağlantı zaman aşımı (timeout) - ses sunucu bağlantısı karışık")
        except Exception as e:
            logger.error(f"❌ Ses kanalına bağlanırken hata: {type(e).__name__}: {str(e)[:100]}")
    ...
    async def voice_reconnect_monitor(self):
        """Ses bağlantısı monitoring ve otomatik yeniden bağlanma (60 saniye)"""
        try:
            guild_id = self.config.get("guild_id")
            if not guild_id:
                return
            ...
            if guild.voice_client and guild.voice_client.is_connected():
                logger.debug("✅ Ses bağlantısı açık")
                self.active_voice_connections[guild_id] = guild.voice_client
            else:
                # Bağlantı yok veya kopmuş
                if guild_id in self.active_voice_connections:
                    del self.active_voice_connections[guild_id]
                
                logger.warning("⚠️  Ses bağlantısı kopmuş veya yok!")
                logger.info("🔄 Bağlantı geri yükleniyor...")
                await self.connect_to_voice_channel()
        except Exception as e:
    ...
    async def check_other_bot(self):
        ...
        try:
            ...
            if not other_status or not other_status.get("online"):
                logger.warning(f"⚠️  Guard Bot 2 offline durumda, Bot 1 korumaya devam ediyor")
            else:
                logger.info("✅ Guard Bot 2 aktif")
